...
### Task 7: Schema Handoff
- **Description:** Commit and push `data/schema.md` so that the ETL script can be written against the correct schema.
- **Dependencies:** Task 1
- **Definition of Done:** `data/schema.md` is on `main` branch.
...
- **Dependencies:** Task 7 (schema discovery complete)
...
- **Description:** Select top 3 most compelling findings. Verify each against source filings (anti-hallucination check). Provide person IDs and narrative for demo flow.
...
- **Description:** Create git tag `v1.0.0-demo` on main. Verify clean `git status`.
- **Dependencies:** All tasks complete
- **Definition of Done:** Tag `v1.0.0-demo` exists on origin/main; no uncommitted changes.
- **Estimated Time:** 5 minutes
- **Files:** None
